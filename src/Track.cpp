...
Track::Track(const string &category, const string &identity, const double speed, const double distance, int id) :
        ship_id_(id), category_(category),
        identity_(identity), speed_(speed),
        distance_(distance), time_elapsed_from_creation{0},
        correlated_track_id{0} {
    cout << "Constructing Track" << endl;
    cout << "Track " << id_ << " created." << endl;
...
Track::Track(const string &category, const string &identity, const double speed) :
        Track(category, identity, speed, 0, ++id_) {
}
